...
def process_class_folder(class_dir: Path):
    ...
    # Determine output directory: ../processed/<class_name>
    # We assume class_dir is something like .../raw/<class_name>
    # So we go up two levels from class_dir to find 'raw', then switch to 'processed'
    
    # Ideally, we allow the user to specify output, but for now let's enforce a standard structure
    # if the input is .../raw, output is .../processed
    
    if class_dir.parent.name == "raw":
        out_root = class_dir.parent.parent / "processed"
    else:
        # Fallback if structure isn't exactly as expected
        out_root = class_dir.parent / "processed"
        
    out_dir = out_root / class_dir.name
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"\nProcessing class: {class_dir.name} -> {out_dir}")
    count = 0
