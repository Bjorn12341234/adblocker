(() => {
  ...
  function findBlockAncestor(el) {
    ...
    while (current && current !== document.body && steps < 8) {
      ...
      if (["li", "article", "section"].includes(tag)) return current;
      if (tag === "div") {
        const rect = current.getBoundingClientRect();
        // Stop if this div spans most of the viewport — it's the feed, not a card
        if (rect.width > viewportWidth * 0.8) return best;
        if (rect.width > 200) {
          best = current;
  ...
  async function filterContent(keywords, settings = {}, extraSelectors = []) {
    ...
    if (settings.sensitivity !== "pictures-only") {
      ...
      // Universal headline scan — works on any site
      document.querySelectorAll("h1, h2, h3, h4, a[href]").forEach(el => {
        if (el.dataset.orangeFilterHidden === "true") return;
        if (el.closest("[data-orange-filter-hidden='true']")) return;
        if (!matchesKeywords(el, allKeywords)) return;
        const target = findBlockAncestor(el);
        if (target.dataset.orangeFilterHidden === "true") return;
        hideElement(target, "Headline matched keywords");
      });
    }
